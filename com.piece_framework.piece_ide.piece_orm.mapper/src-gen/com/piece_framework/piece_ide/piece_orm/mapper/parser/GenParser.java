...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private Piece_ORM_MapperLexer lexer;
	private Piece_ORM_MapperParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public Piece_ORM_MapperLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new Piece_ORM_MapperLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public Piece_ORM_MapperParser getParser() {
		if (parser == null) {
			parser = new Piece_ORM_MapperParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "com::piece_framework::piece_ide::piece_orm::mapper::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"com::piece_framework::piece_ide::piece_orm::mapper::GenChecks","com::piece_framework::piece_ide::piece_orm::mapper::Checks"};
